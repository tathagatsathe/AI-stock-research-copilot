...
class MacroInstabilityService:
    ...
    def snapshot(self) -> dict[str, Any]:
        """
        Fetch VIX history and derive a 1–10 instability score (higher = more stress).

        On failure, returns neutral-ish defaults + coverage low (mirrors news fallback pattern).
        """
        try:
            vix = yf.Ticker(VIX_SYMBOL)
            history = vix.history(period=self._HISTORY_PERIOD, interval=self._HISTORY_INTERVAL, auto_adjust=False)
        except Exception as exc:
            logger.warning("VIX history fetch failed: %s", exc)
            return self._fallback_payload("Failed to fetch VIX history from Yahoo Finance.")

        if history.empty or "Close" not in history:
            return self._fallback_payload("VIX history was empty.")

        close = history["Close"].dropna()
        if close.empty:
            return self._fallback_payload("VIX close series was empty.")
    ...
    @staticmethod
    def _fallback_payload(error_message: str) -> dict[str, Any]:
        return {
            "source": SOURCE,
            "symbol": VIX_SYMBOL,
            "vix_level": None,
